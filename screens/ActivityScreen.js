...
const ButtonSection = ({
  ...
  const handleClearActivity = () => {
    const successTitle = "Clear Activity";
    const successMessage = "Are you sure you want to clear your activity progress? This action cannot be undone.";
    showModal(successTitle, successMessage, () => {
      clearActivity();
      navigateToDashboard();
    }, 'success', [
      { label: "View Dashboard", style: "primary", action: () => { navigateToDashboard(); return true; } },
      { label: "Clear Activity", style: "secondary", action: () => { clearActivity(); return true; } },
    ]);
  };

  // NEW: Only show Save/Clear when not finalized and not viewing a past session
  const shouldShowSaveButtons =
    !alreadySaved &&
  ...
  return (
    <View
      style={[
        twrnc`w-full mb-8`,
        { paddingHorizontal: responsivePadding.base * 4, marginTop: responsivePadding.lg * 4 },
      ]}
    >
        ...
        <View style={[twrnc`flex-col`, { gap: responsivePadding.sm * 4 }]}>
          ...
          <Animated.View style={{ transform: [{ scale: saveScale }] }}>
            <TouchableOpacity
              ...
              disabled={isTrackingLoading || alreadySaved || isViewingPastActivity} // extra safety
              activeOpacity={1}
            >
              <View style={twrnc`flex-row items-center`}>
                <View
                  style={[
                    twrnc`bg-white bg-opacity-20 rounded-full mr-4`,
                ...
                <View style={twrnc`flex-1`}>
                  ...
                  <CustomText
                    style={[twrnc`text-white text-opacity-80`, { fontSize: responsiveFontSizes.sm }]}
                  >
                    {isTrackingLoading ? "Please wait..." : "Save your current session progress"}
                  </CustomText>
                </View>
                {!isTrackingLoading && (
                  <Ionicons
                    name="chevron-forward"
                    size={responsiveSizes.iconSmall}
                    color="#FFFFFF"
                    style={twrnc`opacity-60`}
                  />
                )}
              </View>
            </TouchableOpacity>
          ...
          <Animated.View style={{ transform: [{ scale: clearScale }] }}>
            ...
            >
              <View style={twrnc`flex-row items-center`}>
                <View
                  style={[
                    twrnc`bg-[#EF476F] bg-opacity-20 rounded-full mr-4`,
                    { padding: responsivePadding.sm * 2 },
                  ]}
                >
                ...
                <View style={twrnc`flex-1`}>
                  <CustomText
                    weight="bold"
                    style={[twrnc`text-[#EF476F] mb-1`, { fontSize: responsiveFontSizes.lg }]}
                  >
                    Clear Activity
                  </CustomText>
                  <CustomText
                    style={[twrnc`text-[#EF476F] text-opacity-80`, { fontSize: responsiveFontSizes.sm }]}
                  >
          ...
          <Animated.View style={{ transform: [{ scale: startScale }] }}>
            ...
            >
              <View style={twrnc`flex-row items-center`}>
                <View
                  style={[
                    twrnc`bg-white bg-opacity-25 rounded-full mr-5`,
                ...
                <View style={twrnc`flex-1`}>
                  <CustomText
                    weight="bold"
                    style={[twrnc`text-white mb-1`, { fontSize: responsiveFontSizes.xl }]}
                  >
                    {activeQuest ? "Start Quest" : `Start ${isStrengthActivity ? "Counting" : "Tracking"}`}
                  </CustomText>
                  <CustomText
                    style={[twrnc`text-white text-opacity-90`, { fontSize: responsiveFontSizes.base }]}
                  >
...
const ActivityScreen = ({ navigateToDashboard, navigateToMap, params = {} }) => {
  ...
  const [distance, setDistance] = useState(String(Number(params.distance) || 0)) // Ensure distance is string for TextInput
  const [time, setTime] = useState(String(Number(params.time) || 0)) // Ensure time is string for TextInput
  ...
  const [isViewingPastActivity, setIsViewingPastActivity] = useState(!!(params?.isViewingPastActivity));
  ...
  const [alreadySaved, setAlreadySaved] = useState(!!(params?.alreadySaved)); // NEW [ActivityScreen.js]
  // NEW/REPLACE [ActivityScreen.js]

  // NEW: State to control countdown visibility
  ...
  // FIX: Enhanced quest initialization from params (received from Dashboard)
  // Replace the quest initialization useEffect
  useEffect(() => {
    console.log("ActivityScreen: Processing params for quest initialization:", params);

    // Clear any existing quest if no quest data in params
    if (!params.questId && !params.title) {
      console.log("ActivityScreen: No quest data found in params, clearing activeQuest");
      setActiveQuest(null);
      return;
    }
    ...
    // ONLY set activity type if the quest specifies one AND it's valid
    if (questData.activityType && activities.some(a => a.id === questData.activityType)) {
      console.log("ActivityScreen: Updating selectedActivity from quest:", questData.activityType);
  ...
  const startAccelerometerTracking = useCallback(() => {
    ...
    let threshold = 1.2; // Default threshold
    let resetThreshold = 0.8; // Default reset threshold
  ...
  const saveActivity = useCallback(async (finalStats = null) => { // NEW: Accept finalStats from MapScreen or FaceProximityPushUpCounter
    console.log("ActivityScreen: Starting save activity process")
    setIsTrackingLoading(true)

    // GUARD: prevent duplicate XP on finalized/past sessions
    if (alreadySaved || isViewingPastActivity) {
      showModal(
        "Already saved",
        "This session was already finalized. Start a new activity to earn XP again.",
        () => { },
        "info"
      );
    ...
    // Use finalStats if provided, otherwise use local 'stats'
    const activityStatsToSave = finalStats || stats || { reps: 0, duration: 0, distance: 0, steps: 0 };

    const normalizedStats = {
      reps: Number(activityStatsToSave.reps) || 0,
      duration: Number(activityStatsToSave.duration) || 0,
      distance: Number(activityStatsToSave.distance) || 0,
      steps: Number(activityStatsToSave.steps) || 0,
      sets: Number(activityStatsToSave.sets) || 1,
      restTime: Number(activityStatsToSave.restTime) || 0,
      maxSpeed: Number(activityStatsToSave.maxSpeed) || 0,
      elevationGain: Number(activityStatsToSave.elevationGain) || 0,
      elevationLoss: Number(activityStatsToSave.elevationLoss) || 0,
    };
    console.log("ActivityScreen: Normalized stats for saving:", normalizedStats)
    ...
    try {
      ...
      if (isStrengthActivity) {
        ...
        if (activityStatsToSave.reps < 5) { // NEW: Use activityStatsToSave
          showModal(
            "Activity Too Short",
        ...
        if (activityStatsToSave.distance / 1000 < minDistance) { // NEW: Use activityStatsToSave
          showModal(
            "Activity Too Short",
        ...
        if (activityStatsToSave.duration < minDuration) { // NEW: Use activityStatsToSave
          showModal(
            "Activity Too Short",
      ...
      if (!isStrengthActivity) {
        const distanceInKm = normalizedStats.distance / 1000;
        const durationInHours = normalizedStats.duration / 3600;
        const durationInMinutes = normalizedStats.duration / 60;
        calculatedMetrics = {
          distanceInKm,
      ...
      const baseActivityData = {
        ...
        duration: activityStatsToSave.duration || 0, // NEW: Use activityStatsToSave
        calories: activityStatsToSave.calories || calories || 0, // Use calories from finalStats if available, else local state
      ...
      const activityData = isStrengthActivity
        ? {
          ...baseActivityData,
          reps: activityStatsToSave.reps || 0, // NEW: Use activityStatsToSave
          // Add strength-specific fields
          sets: activityStatsToSave.sets || 1,
          restTime: activityStatsToSave.restTime || 0,
        }
        : {
          ...baseActivityData,
          distance: activityStatsToSave.distance || 0, // in meters // NEW: Use activityStatsToSave
          steps: activityStatsToSave.steps || 0, // NEW: Use activityStatsToSave
          avgSpeed: calculatedMetrics.avgSpeed || 0, // km/h
          pace: calculatedMetrics.pace || 0, // min/km
          maxSpeed: activityStatsToSave.maxSpeed || 0, // NEW: Use activityStatsToSave
          elevationGain: activityStatsToSave.elevationGain || 0, // NEW: Use activityStatsToSave
          elevationLoss: activityStatsToSave.elevationLoss || 0, // NEW: Use activityStatsToSave
      ...
      if (activeQuest) {
        ...
        if (activeQuest.unit === "reps" && isStrengthActivity) {
          currentValue = activityStatsToSave.reps || 0 // Use activityStatsToSave
          ...
          currentValue = (activityStatsToSave.duration || 0) / 60 // Convert to minutes
          questProgress = currentValue / activeQuest.goal
        } else if (activeQuest.unit === "steps" && !isStrengthActivity) {
          currentValue = activityStatsToSave.steps || 0 // Use activityStatsToSave
          questProgress = currentValue / activeQuest.goal
        } else if (activeQuest.unit === "calories") {
          currentValue = activityStatsToSave.calories || calories || 0; // Use calories from finalStats if available
          questProgress = currentValue / activeQuest.goal;
        }
        ...
        if (questCompleted) {
          ...
          questCompletionData = {
            ...
            activityData: {
              ...(isStrengthActivity
                ? {
                  reps: activityStatsToSave.reps || 0, // Use activityStatsToSave
                  duration: activityStatsToSave.duration || 0, // Use activityStatsToSave
                }
                : {
                  distance: calculatedMetrics.distanceInKm || 0,
                  duration: (activityStatsToSave.duration || 0) / 60, // Use activityStatsToSave
                  avgSpeed: calculatedMetrics.avgSpeed || 0,
                  steps: activityStatsToSave.steps || 0, // Use activityStatsToSave
                }),
            },
          }
      ...
      if (isStrengthActivity) {
        ...
        bonusXP += Math.floor((activityStatsToSave.reps || 0) / 10) * 5 // 5 XP per 10 reps // Use activityStatsToSave
        bonusXP += Math.floor((activityStatsToSave.duration || 0) / 300) * 10 // 10 XP per 5 minutes // Use activityStatsToSave
      } else {
        // Bonus XP for distance activities
        bonusXP += Math.floor(calculatedMetrics.distanceInKm || 0) * 10 // 10 XP per km
        bonusXP += Math.floor((activityStatsToSave.duration || 0) / 600) * 5 // 5 XP per 10 minutes // Use activityStatsToSave
        bonusXP += Math.floor((activityStatsToSave.steps || 0) / 1000) * 2 // 2 XP per 1000 steps // Use activityStatsToSave
      }
      xpEarned += bonusXP
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (isStrengthActivity) {
            userUpdateData.totalReps = (userData.totalReps || 0) + (activityStatsToSave.reps || 0) // Use activityStatsToSave
            userUpdateData.totalStrengthDuration = (userData.totalStrengthDuration || 0) + (activityStatsToSave.duration || 0) // Use activityStatsToSave
          } else {
            userUpdateData.totalDistance = (userData.totalDistance || 0) + (calculatedMetrics.distanceInKm || 0)
            userUpdateData.totalSteps = (userData.totalSteps || 0) + (activityStatsToSave.steps || 0) // Use activityStatsToSave
            userUpdateData.totalDuration = (userData.totalDuration || 0) + (activityStatsToSave.duration || 0) // Use activityStatsToSave
          }
      ...
      let successMessage = ""
      const safeDistance = calculatedMetrics.distanceInKm || 0;
      const safeAvgSpeed = calculatedMetrics.avgSpeed || 0;
      const safeSteps = normalizedStats.steps || 0;
      const safeReps = normalizedStats.reps || 0;
      const safeDuration = normalizedStats.duration || 0;
      const safeCalories = activityStatsToSave.calories || calories || 0; // Use calories from finalStats if available

      if (questCompleted) {
        if (isStrengthActivity) {
          successMessage = `Congratulations! You completed the "${activeQuest?.title}" quest and earned ${xpEarned} XP!\n\nWorkout Summary:\n• ${safeReps} ${selectedActivity} reps\n• ${formatDuration(safeDuration)} duration\n• ${safeCalories} calories burned`;
        } else {
          successMessage = `Congratulations! You completed the "${activeQuest?.title}" quest and earned ${xpEarned} XP!\n\nWorkout Summary:\n• ${safeDistance.toFixed(2)} km distance\n• ${formatDuration(safeDuration)} duration\n• ${safeAvgSpeed.toFixed(1)} km/h avg speed\n• ${safeSteps.toLocaleString()} steps\n• ${safeCalories} calories burned`;
        }
      } else {
        if (isStrengthActivity) {
          successMessage = `Awesome ${selectedActivity} session!\n\nWorkout Summary:\n• ${safeReps} reps completed\n• ${formatDuration(safeDuration)} duration\n• ${safeCalories} calories burned\n• ${xpEarned} XP earned`;
        } else {
          successMessage = `Great ${selectedActivity} session!\n\nWorkout Summary:\n• ${safeDistance.toFixed(2)} km covered\n• ${formatDuration(safeDuration)} duration\n• ${safeAvgSpeed.toFixed(1)} km/h avg speed\n• ${safeSteps.toLocaleString()} steps\n• ${safeCalories} calories burned\n• ${xpEarned} XP earned`;
        }
      }
    ...
    // NEW: Update challenge progress if applicable


    if (activeQuest && activeQuest.category === "challenge") {
      try {
        const challengeRef = doc(db, "challenges", activeQuest.id);
        ...
        if (challengeDoc.exists()) {
          ...
          if (!currentUser) {
            console.warn("User  not authenticated, cannot update challenge progress");
            return;
          }
          ...
          // Calculate value to add based on unit and activity type
          let valueToAdd = 0;
          if (activeQuest.unit === "reps" && isStrengthActivity) {
            valueToAdd = activityStatsToSave.reps || 0;
          } else if (activeQuest.unit === "distance" && !isStrengthActivity) {
            valueToAdd = (activityStatsToSave.distance || 0) / 1000; // meters to km
          } else if (activeQuest.unit === "duration") {
            valueToAdd = (activityStatsToSave.duration || 0) / 60; // seconds to minutes
          } else if (activeQuest.unit === "steps" && !isStrengthActivity) {
            valueToAdd = activityStatsToSave.steps || 0;
          } else if (activeQuest.unit === "calories") {
            valueToAdd = activityStatsToSave.calories || calories || 0; // Use calories from finalStats if available
          }

          // Update progress safely
          // This is the crucial part: add current session's value to existing progress
          const newProgressValue = (progress[userId] || 0) + valueToAdd;

          // Determine new status
          const newStatusValue = goal > 0 && newProgressValue >= goal ? "completed" : "in_progress";

          // Update Firestore document with only defined values
          await updateDoc(challengeRef, {
            [`progress.${userId}`]: newProgressValue,
          });

          await updateDoc(challengeRef, {
            [`status.${userId}`]: newStatusValue,
          });
        }
      } catch (err) {
    ...
    formatDuration,
    showModal,
    stats, // Added stats to dependencies as activityStatsToSave is derived from it
  ])
  ...
  const resumeTracking = useCallback(() => {
    ...
    navigateToMap({
      ...
      targetDistance: Number(distance) || 0, // Convert to number for MapScreen
      targetTime: Number(time) || 0, // Convert to number for MapScreen
      tracking: true,
  ...
  // NEW: Modified startActivity to trigger countdown
  const startActivity = useCallback(() => {
    console.log("ActivityScreen: Initiating countdown for activity:", selectedActivity);
    const activityConfig = activities.find((a) => a.id === selectedActivity);
    ...
    // Reset stats before starting countdown
    setStats({
      distance: 0,
      duration: 0,
      pace: 0,
  ...
  // NEW: Callback function after countdown finishes
  const handleCountdownFinish = useCallback(() => {
    setShowCountdown(false); // Hide the countdown overlay
    console.log("ActivityScreen: Countdown finished, proceeding to activity start logic.");
    ...
    navigateToMap({
      ...
      tracking: false, // MapScreen will handle starting tracking after its own setup
      initialCoordinates: [],
  ...
  const getDisplayQuestProgress = useCallback(
    (quest) => {
      ...
      const initialProgressValue = (quest.progress || 0) * (quest.goal || 1); // Convert initial progress percentage to a value
      ...
      if (quest.unit === "steps") {
        ...
        currentSessionValue = Number.parseFloat(stats.distance || 0) / 1000; // Convert meters to km
      } else if (quest.unit === "duration") {
        currentSessionValue = stats.duration / 60; // Convert seconds to minutes
      } else if (quest.unit === "calories") {
        currentSessionValue = calories; // Use current session's calories
      }

      const totalAchievedValue = initialProgressValue + currentSessionValue;
      const goalValue = Number.parseFloat(quest.goal || 0);

      const calculatedProgress = goalValue > 0 ? Math.min(totalAchievedValue / goalValue, 1) : 0;
    ...
    [stats.steps, stats.distance, stats.reps, stats.duration, calories], // Add calories to dependencies
  );
  ...
  const getCurrentQuestValue = useCallback(
    ...
    [stats.steps, stats.reps, stats.distance, stats.duration, calories], // Add calories to dependencies
  );
  ...
  // NEW: Callback function after FaceProximityPushUpCounter finishes
  const handleFaceCounterFinish = useCallback((finalStatsFromCounter) => { // Accept finalStats
    setShowFaceCounter(false);
    console.log("ActivityScreen: FaceProximityPushUpCounter finished. Final stats:", finalStatsFromCounter);

    // Immediately call saveActivity with the stats received from the counter
    if (finalStatsFromCounter) {
      saveActivity(finalStatsFromCounter);
    } else {
      // Fallback if for some reason finalStatsFromCounter is null/undefined
      console.warn("ActivityScreen: No final stats received from FaceProximityPushUpCounter. Saving current stats as fallback.");
      saveActivity(); // Call with no args, so it uses local 'stats'
    }
  }, [saveActivity]); // Add saveActivity to dependencies
  ...
  // NEW: Render CountdownOverlay if showCountdown is true
  if (showCountdown) {
    return <CountdownOverlay onCountdownFinish={handleCountdownFinish} />;
  }
    ...
    <View style={twrnc`flex-1 bg-[#121826]`}>
      ...
      <View style={twrnc`bg-[#1A1F2E] px-4 py-4 flex-row items-center justify-between shadow-lg`}>
        ...
        <TouchableOpacity
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
        >
          <Ionicons name={showSettings ? "close" : "settings-outline"} size={20} color="#FFFFFF" />
        </TouchableOpacity>
      </View>
              ...
              <View style={[
                ...
                <View style={[twrnc`flex-row items-center`, { marginBottom: responsivePadding.sm * 3 }]}>
                  <View style={[
                    ...
                    {
                      padding: responsivePadding.sm * 2,
                      width: responsiveSizes.iconMedium * 1.8, // Make it a bit larger
                      height: responsiveSizes.iconMedium * 1.8,
                      alignItems: 'center',
                        ...
                        <TouchableOpacity
                          ...
                          style={[
                            twrnc`items-center rounded-lg`,
                            {
                              ...
                              paddingVertical: responsivePadding.sm * 2,
                              backgroundColor: time === String(mins) ? currentActivity.color : "#3A3F4B" // Compare with string
                            }
                          ]}
                          onPress={() => setTime(String(mins))} // Set as string
                        >
