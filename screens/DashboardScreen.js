...
const DashboardScreen = ({ navigateToActivity, navigation, userData }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (timePeriod === "week") {
        ...
        // Remove `concentración` from here since it's not a valid parameter
        endDate.setHours(23, 59, 59, 999);  // Corrected line
      }
      ...
      // Process weekly and monthly progress (unchanged from original)
      ...
      if (todayQuest) {
        const weekProgress = weekDates.map(({ date }) => {
          ...
          if (todayQuest.unit === "steps") {
            ...
            totalValue = dayActivities.reduce((sum, act) => sum + (Number(act.distance) || 0), 0);
  ...
  const calculateQuestProgress = (quest) => {
    ...
    if (quest.unit === "steps") {
      ...
      currentValue = Number.parseInt(durationParts[0]) * 60 + Number.parseInt(durationParts[1]) || 0;
  ...
  const getCurrentQuestValue = (quest) => {
    ...
    if (quest.unit === "steps") {
      return Math.min(Number(activityData.steps) || 0, quest.goal);
    } else if (quest.unit === "distance") {
      const currentDistance = Number.parseFloat(activityData.distance) || 0;
      return Math.min(currentDistance, quest.goal);
    } else if (quest.unit === "duration") {
      const durationParts = activityData.duration.split(":");
      const currentMinutes = Number.parseInt(durationParts[0]) * 60 + Number.parseInt(durationParts[1]) || 0;
  ...
  return (
    <View style={twrnc`flex-1 bg-[#121826]`}>
                    ...
                    >
                      ...
                      <View style={twrnc`flex-row items-start mb-4`}>
                        ...
                        <View style={twrnc`flex-1`}>
                          ...
                          <CustomText
                            style={twrnc`text-gray-400 text-sm mb-2`}
                            numberOfLines={2}
                          >
                            {quest.description}
                          </CustomText>
      ...
      >
        <Pressable
          style={twrnc`flex-1 bg-black/70 justify-end`}
            ...
            <Pressable onPress={(e) => e.stopPropagation()}>
              <View style={twrnc`p-6`}>
                ...
                {selectedActivity?.coordinates?.length > 0 ? (
                  ...
                  <View style={twrnc`w-full h-40 bg-gray-800 justify-center items-center rounded-2xl mb-6 border border-dashed border-gray-600`}>
                    <Ionicons name="map-outline" size={36} color="#6B7280" style={twrnc`mb-2`} />
                    <CustomText style={twrnc`text-gray-400 text-sm text-center`}>
                ...
                {selectedActivity && (
                  <View style={twrnc`bg-gray-800 rounded-2xl p-5 mb-6`}>
                    ...
                    <View style={twrnc`flex-row justify-between mb-5`}>
                      <View style={twrnc`items-center flex-1`}>
                        <Ionicons name="location-outline" size={16} color="#9CA3AF" style={twrnc`mb-1`} />
                        <CustomText style={twrnc`text-gray-400 text-xs mb-1`}>Distance</CustomText>
                        <CustomText weight="bold" style={twrnc`text-emerald-400 text-xl`}>
                      ...
                      <View style={twrnc`items-center flex-1 border-l border-r border-gray-700`}>
                        <Ionicons name="time-outline" size={16} color="#9CA3AF" style={twrnc`mb-1`} />
                        <CustomText style={twrnc`text-gray-400 text-xs mb-1`}>Duration</CustomText>
                        <CustomText weight="bold" style={twrnc`text-amber-400 text-xl`}>
                          {selectedActivity.duration
                      ...
                      <View style={twrnc`items-center flex-1`}>
                        <Ionicons
                          name={selectedActivity.activityType === "cycling" ? "speedometer-outline" : "walk-outline"}
                          size={16}
                          color="#9CA3AF"
                          style={twrnc`mb-1`}
                        />
                        ...
                        <CustomText weight="bold" style={twrnc`text-blue-400 text-xl`}>
                    ...
                    <View style={twrnc`flex-row justify-between`}>
                      <View style={twrnc`flex-1 mr-4`}>
                        ...
                        <View style={twrnc`flex-row items-center`}>
                          <Ionicons
                            name={
                              selectedActivity.activityType === "running" ? "footsteps-outline" :
                                selectedActivity.activityType === "cycling" ? "bicycle-outline" :
                                  "walk-outline"
                            }
                            size={16}
                            color="#FFFFFF"
